...
fn make_side_panel(
    ...
    for idx in 0..canonical_signal.stages.len() {
        ...
        let (default_batch, bounds) = Image::from_batch(icon_batch.clone(), icon_bounds.clone())
            .color(RewriteColor::NoOp)
            .padding(16)
            .build_batch(ctx)
            .unwrap();
        let (hovering_batch, _) = Image::from_batch(icon_batch.clone(), icon_bounds.clone())
            .color(RewriteColor::NoOp)
            .padding(16)
            .bg_color(ctx.style().btn_tab.bg_disabled.dull(0.3))
            .build_batch(ctx)
            .unwrap();
        let (selected_batch, _) = Image::from_batch(icon_batch.clone(), icon_bounds.clone())
            //.color(RewriteColor::NoOp)
            .padding(123)
            //.bg_color(ctx.style().btn_tab.bg_disabled)
